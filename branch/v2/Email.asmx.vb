...
Public Class Email
    ...
    Public Function SendMail(ByVal objMail As EmailInfo) As BaseResponse
        If _Log.IsDebugEnabled Then _Log.Debug("sendemail entered...")
        ...
        Try
            ...
            Else
                ...
                message.From = New MailAddress(objMail.MailFrom)
                '*** now add additional TO addresses
                If _Log.IsDebugEnabled Then _Log.Debug("mail to:" & objMail.MailTo)
                Dim toemails() As String = Split(objMail.MailTo, ",")
                If toemails.Length > 1 Then
                    For x As Integer = 0 To toemails.Length - 1
                        If _Log.IsDebugEnabled Then _Log.Debug(x & ":" & toemails(x))
                        message.To.Add(toemails(x))
                    Next
                Else
                    message.To.Add(objMail.MailTo)
                End If
